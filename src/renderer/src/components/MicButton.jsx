...
export default function MicButton({ onStatusChange }) {
  ...
  // Setup the AI Worker once on mount
  useEffect(() => {
  ...
  // --- THE "ONLY ON HOLD" LOGIC ---
  
  const startListening = async () => {
    try {
      // 1. Only request the mic when the button is actually pressed
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true })
      streamRef.current = stream

      // 2. Setup Analyser for the "Dancing" effect
      ...
      recorder.onstop = async () => {
        ...
        // Kill the stream and analyser to stop listening
  ...
  return (
    <div 
      ...
      onMouseLeave={stopListening} // Safety: stop if mouse slides off button
    ...
    >
      ...
      <div className={`relative z-10 flex h-60 w-60 items-center justify-center rounded-full border transition-all duration-500 
        ...
        <div className="text-center z-20">
          ...
          <p className={`text-xl font-black tracking-widest uppercase transition-colors duration-300 ${status === 'listening' ? 'text-red-500' : 'text-cyan-400'}`}>
